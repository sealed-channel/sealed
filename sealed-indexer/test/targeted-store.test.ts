...
describe('TargetedPushTokenStore', () => {
  ...
  it('round-trips an optional wallet (KEM push opt-in)', () => {
    const store = createTargetedPushTokenStore(dbPath);
    const kp = genKeypair();
    ...
    expect(store.get(HEX_BLINDED)?.wallet).toBe(wallet);
    store.close();
  });

  it('defaults wallet to null when omitted', () => {
    const store = createTargetedPushTokenStore(dbPath);
    const kp = genKeypair();
    store.register({
      blindedId: HEX_BLINDED,
    ...
    expect(store.get(HEX_BLINDED)?.wallet).toBeNull();
    store.close();
  });
