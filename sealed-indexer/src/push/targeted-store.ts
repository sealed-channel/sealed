...
export interface TargetedRegistration {
  ...
  viewPub: Buffer;
  /**
   * Recipient's Algorand wallet address (base32). Optional: only present for
   * clients that opted into KEM first-contact push. Required by the fanout's
   * KEM-tag branch; null rows simply never match a KEM handshake frame.
   */
  wallet: string | null;
  createdAt: number;
  updatedAt: number;
...
// Algorand addresses are 58-char uppercase base32 (RFC4648, no padding) — A–Z,2–7.
...
export function createTargetedPushTokenStore(dbPath: string): TargetedPushTokenStore {
  ...
  // Migration: add the nullable `wallet` column for KEM first-contact push.
  // Idempotent — skip if a prior run already added it.
  const hasWallet = (db.prepare(`PRAGMA table_info(targeted_push_tokens)`).all() as Array<{
    name: string;
  }>).some((c) => c.name === 'wallet');
  if (!hasWallet) {
    db.exec(`ALTER TABLE targeted_push_tokens ADD COLUMN wallet TEXT`);
  }

  const upsertStmt = db.prepare(`
    INSERT INTO targeted_push_tokens
      (blinded_id, enc_token, platform, view_priv, view_pub, wallet, created_at, updated_at)
    VALUES (?, ?, ?, ?, ?, ?, ?, ?)
  ...
  return {
    register(input) {
      ...
      // Wallet is optional. When present it must be a 58-char Algorand base32
      // address (the format used to derive the KEM discovery tag). Reject
      // garbage rather than store a value that can never match a real frame.
      const wallet = input.wallet ?? null;
      if (wallet !== null && !ALGORAND_ADDRESS.test(wallet)) {
        throw new Error('Invalid wallet: must be a 58-char Algorand address');
      }
