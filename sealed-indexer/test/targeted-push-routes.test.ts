/**
 * Tests for /push/register and /push/unregister.
 *
...
describe('HTTP /push/register', () => {
  ...
  it('accepts a well-formed targeted registration', async () => {
    const app = createApp();
    const res = await request(app).post('/push/register').send(makeReq());
...
describe('HTTP /push/unregister', () => {
  it('accepts a valid unregister', async () => {
    const app = createApp();
    const res = await request(app).post('/push/unregister').send({
