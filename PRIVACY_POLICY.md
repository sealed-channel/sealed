...
- All message content is encrypted end-to-end using AES-256-GCM with per-message ephemeral keys derived via hybrid X25519 + ML-KEM-512 with HKDF
...
**Post-quantum encryption:** Message keys are derived from two independent secrets — one classical (X25519) and one post-quantum (ML-KEM-512, also known as Kyber-512) — combined via HKDF. Message confidentiality therefore does not rely on a single key-exchange primitive against future quantum-capable adversaries.
