...
void main() {
  group('ChainAddress', () {
    ...
    // =========================================================================
    group('isValid', () {
      test('valid 58-char algorand address returns true', () {
        final addr = ChainAddress.encode(validEd25519PubKey(0x42), 'algorand');
        expect(ChainAddress.isValid(addr, 'algorand'), isTrue);
      });

      test('short string returns false', () {
        expect(ChainAddress.isValid('abc', 'algorand'), isFalse);
      });

      test('empty string returns false', () {
        expect(ChainAddress.isValid('', 'algorand'), isFalse);
      });

      test('unknown chain returns false, does not throw', () {
        final addr = ChainAddress.encode(validEd25519PubKey(0x42), 'algorand');
        expect(ChainAddress.isValid(addr, 'solana_fake'), isFalse);
      });

      test('corrupted address returns false', () {
        final addr = ChainAddress.encode(validEd25519PubKey(0x01), 'algorand');
        final corrupted = 'A' + addr.substring(1);
        // may or may not be false depending on checksum; just must not throw
        expect(
          () => ChainAddress.isValid(corrupted, 'algorand'),
          returnsNormally,
        );
      });
    });
