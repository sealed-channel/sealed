/**
 * Mirror of programs/sealed/src/lib/code.ts.
 *
 * Code format: 16-char Crockford base32 (no I, L, O, U). 80 bits entropy.
 * Preimage   : first 16 bytes of sha256(utf8(normalized code)).
 * Commitment : sha256(preimage). Posted on-chain by admin and matched by
 *              the contract's `redeem` against `c:<sha256(preimage)>`.
 *
 * Normalization: uppercase, strip dashes/spaces, map I→1, L→1, O→0.
 * Display form may include dashes: `XXXX-XXXX-XXXX-XXXX`.
 *
 * KEEP IN SYNC with the contract-side module. Drift here = unredeemable codes.
 * Parity is enforced by golden vectors in test/preimage-server/codes.test.ts.
 */
...
export function commitmentHash(preimage: Uint8Array): Uint8Array {
