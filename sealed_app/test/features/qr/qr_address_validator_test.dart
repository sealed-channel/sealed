...
void main() {
  ...
  group('extractAlgorandAddress', () {
    final valid = _addressFor(
      List<int>.generate(32, (i) => (i * 17 + 3) & 0xff),
    );

    test('accepts bare address', () {
      expect(extractAlgorandAddress(valid), valid);
    });

    test('accepts bare address with surrounding whitespace', () {
      expect(extractAlgorandAddress('  $valid\n'), valid);
    });

    test('accepts algorand:// URI (Pera/Defly)', () {
      expect(extractAlgorandAddress('algorand://$valid'), valid);
    });

    test('accepts algorand: URI without slashes', () {
      expect(extractAlgorandAddress('algorand:$valid'), valid);
    });

    test('accepts algorand URI with query params (payment request)', () {
      expect(
        extractAlgorandAddress('algorand://$valid?amount=1000&note=hi'),
        valid,
      );
    });
    ...
    test('accepts legacy Pera JSON receive QR', () {
      expect(
        extractAlgorandAddress('{"version":"1.0","address":"$valid"}'),
        valid,
      );
    });

    test('rejects URI with invalid address', () {
      expect(extractAlgorandAddress('algorand://NOTANADDRESS'), isNull);
    });

    test('rejects JSON with invalid address', () {
      expect(extractAlgorandAddress('{"address":"NOTANADDRESS"}'), isNull);
    });

    test('rejects malformed JSON', () {
      expect(extractAlgorandAddress('{address}'), isNull);
    });

    test('rejects WalletConnect URIs', () {
      expect(extractAlgorandAddress('wc:abc123@1?bridge=x&key=y'), isNull);
    });

    test('rejects null and empty', () {
      expect(extractAlgorandAddress(null), isNull);
      expect(extractAlgorandAddress(''), isNull);
      expect(extractAlgorandAddress('   '), isNull);
    });
  });
}
