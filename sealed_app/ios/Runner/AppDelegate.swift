...
@objc class AppDelegate: FlutterAppDelegate, FlutterImplicitEngineDelegate {
  ...
  func didInitializeImplicitFlutterEngine(_ engineBridge: FlutterImplicitEngineBridge) {
    ...
    // SecureScreen (chat, alias handshake) is mounted. iOS cannot block
    // capture outright; the secure-UITextField layer trick below makes
    // screenshots and recordings of the window render black instead.
    if let messenger = engineBridge.pluginRegistry.registrar(forPlugin: "ScreenSecureChannel")?
      .messenger()
    {
      let channel = FlutterMethodChannel(
        name: AppDelegate.screenSecureChannelName,
        binaryMessenger: messenger
      )
  ...
  private static let screenSecureChannelName = "sealed/screen_secure"

  // Secure-field capture protection. A hidden UITextField with
  // `isSecureTextEntry = true` whose layer hierarchy is spliced around the
  // window layer causes the OS to blank the window in screenshots, screen
  // recordings, and (in practice) the app-switcher snapshot. Undocumented
  // behavior (works iOS 13–18, widely shipped); if a future iOS breaks it
  // the app fails open — capture proceeds, nothing else is affected.
  private var secureField: UITextField?

  // Scene-based lifecycle (FlutterSceneDelegate in Info.plist): the window
  // lives on the window scene, so `self.window` is nil here. Resolve the key
  // window from connected scenes, falling back to the AppDelegate property
  // for non-scene setups.
  ...
  private func setSecureScreen(_ secure: Bool) {
    guard let window = captureWindow else {
      NSLog("[AppDelegate] screen_secure: no window — capture protection unavailable")
      return
    }
    if secureField == nil {
    ...
    // Layer wiring stays in place permanently; the flag alone toggles
    // whether the OS blanks the window.
