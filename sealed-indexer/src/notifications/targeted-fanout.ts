...
import { isMessageForViewKey, computeKemDiscoveryTag, constantTimeEquals } from './view-keys';
import { TARGETED_PUSH_BODY } from './targeted-payload';

/** True when every byte is zero — the client's marker for a KEM handshake frame. */
...
export function createTargetedFanout(opts: TargetedFanoutOptions): TargetedFanout {
  ...
  async function handle(event: AlgorandMessageEvent): Promise<void> {
    ...
    // A zeroed X25519 ephemeral is the client's marker for a KEM handshake
    // frame (first message to a new contact). These carry the deterministic
    // KEM discovery tag, not the ECDH recipient_tag, so they match on
    // HMAC(sender_wallet ‖ recipient_wallet) instead of trial-decrypt.
    const isKemFrame = isAllZero(event.senderEphemeralPubkey);
    const t0 = Date.now();
    logger.info(
      {
        txId: event.txId,
    ...
    if (isKemFrame && !event.sender) {
      logger.warn(
        { txId: event.txId },
        'targeted-fanout: KEM frame without sender address — cannot match, dropping',
      );
      return;
    }
    ...
    for (const reg of regs) {
      ...
      let isMatch = false;
      if (isKemFrame) {
        // KEM handshake frame: match the deterministic discovery tag, keyed on
        // (on-chain sender wallet ‖ this registration's wallet). Rows without a
        // wallet (older clients that didn't opt into KEM push) can't match.
        if (!reg.wallet) continue;
        try {
          const expected = computeKemDiscoveryTag(event.sender!, reg.wallet);
          isMatch = constantTimeEquals(expected, event.recipientTag);
        } catch (err) {
          logger.warn(
            { err, blindedId: reg.blindedId },
            'targeted-fanout: KEM-tag compute threw — skipping registration',
          );
          continue;
        }
      } else {
