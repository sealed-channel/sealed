...
export function createOhttpFcmSender(opts: OhttpFcmSenderOptions): TargetedSender {
  ...
  return {
    async send(args: TargetedDispatchArgs): Promise<{ ok: boolean; status?: number }> {
      ...
      // PUSH_DATA_ONLY=1 → data-only (app decides display; suppresses blocked
      // senders). Default = visible-alert (unchanged, OS-rendered).
      const payload = Buffer.from(
        isPushDataOnly()
          ? buildFcmDataOnlyBody(deviceToken)
          : buildFcmMessageBody(deviceToken),
        'utf8',
      );
      const t0 = Date.now();
