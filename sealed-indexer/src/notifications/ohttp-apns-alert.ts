...
export function createAlertApnsSender(opts: AlertApnsSenderOptions): TargetedSender {
  ...
  return {
    async send(args: TargetedDispatchArgs): Promise<{ ok: boolean; status?: number }> {
      ...
      // PUSH_DATA_ONLY=1 → silent content-available (app decides display);
      // else visible-alert with the frozen body guard.
      const dataOnly = isPushDataOnly();
      if (!dataOnly && body !== TARGETED_PUSH_BODY) {
        throw new Error(
          'visible-alert APNs body must equal the frozen TARGETED_PUSH_BODY constant',
        );
      }

      const payload = dataOnly ? buildApnsSilentBody() : buildApnsBody();
