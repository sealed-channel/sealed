...
// yields identical bytes. The keyed Hmac.sha256() tag path stays native (real
// 32B key, no empty-key risk) — that's the perf-critical sync hot path.
...
import 'package:sealed_app/models/kem.dart';
import 'package:sealed_app/models/message_payload.dart';
...
class CryptoService {
  ...
  generatePqKeyPair() async {
    final seed = secureRandomBytes(64);
  ...
  ///
  /// [nonce] is the 32-byte encapsulation coins. Default = `Random.secure()`
  /// (one-shot, non-recoverable). Pass a **deterministic** nonce (derived from
  /// the mnemonic + recipient PQ pubkey, see `KeyService.deriveKemEncapsNonce`)
  /// to make `(ciphertext, sharedSecret)` reproducible so the initiator can
  /// re-derive the shared secret after a logout/cache wipe. Derandomized KEM:
  /// coins from a secret seed are standard, no security loss.
  Future<KemResult> kemEncapsulate(
    Uint8List recipientPqPubkey, {
    Uint8List? nonce,
  }) async {
    try {
      final kyber = Kyber.kem512();
      final pk = KemPublicKey.deserialize(recipientPqPubkey, 2);
      final coins = nonce ?? secureRandomBytes(32);
      if (coins.length != 32) {
        throw CryptoValidationException('KEM nonce must be 32 bytes');
      }
      final (cipher, sharedSecret) = kyber.encapsulate(pk, coins);
      return KemResult(
  ...
  // ============================================================
  // Spec H — Per-contact stable recipient tag + shared secret.
  // ============================================================
  //
  // Each contact (alias chat) holds a 32B `sharedSecret` derived once via
  // X25519 + ML-KEM-512 (see deriveContactSharedSecret). From that secret we
  // derive:
  //   - one stable `recipient_tag` used for every message in the chat
  //   - one stable `msg_key` used as AES-GCM key (fresh random nonce per send)
  //
  // No per-message counter, no sliding window. New alias chat = new shared
  // secret = new tag. Trade-off: chain observer can cluster all msgs in a
  // chat under one tag. We accept that for indexer simplicity.

  static const _tagInfo = 'sealed-tag-v1';
  static const _msgKeyInfo = 'sealed-msg-v1';

  void _validateSharedSecret(Uint8List ss) {
    if (ss.length != 32) {
      throw CryptoValidationException(
        'sharedSecret must be 32 bytes, got ${ss.length}',
      );
    }
  }

  /// Derive the stable 32B routing tag for a contact.
  ///
  /// `recipient_tag = HKDF-Expand(sharedSecret, info = "sealed-tag-v1", L=32)`
  Future<Uint8List> contactRecipientTag(Uint8List sharedSecret) async {
    _validateSharedSecret(sharedSecret);
    final hkdf = DartHkdf(hmac: DartHmac.sha256(), outputLength: 32);
    final key = await hkdf.deriveKey(
      secretKey: SecretKey(sharedSecret),
      nonce: const <int>[],
      info: utf8.encode(_tagInfo),
    );
    return Uint8List.fromList(await key.extractBytes());
  }

  /// Derive the stable 32B AES-GCM key for a contact.
  Future<Uint8List> contactMessageKey(Uint8List sharedSecret) async {
    _validateSharedSecret(sharedSecret);
    final hkdf = DartHkdf(hmac: DartHmac.sha256(), outputLength: 32);
    final key = await hkdf.deriveKey(
      secretKey: SecretKey(sharedSecret),
      nonce: const <int>[],
      info: utf8.encode(_msgKeyInfo),
    );
    return Uint8List.fromList(await key.extractBytes());
  }

  /// Compute the contact-pair `sharedSecret` from a completed X25519 + ML-KEM
  /// handshake (Spec H §3.1). `tagSalt` binds the secret to this specific
  /// invite to defend against cross-contact reuse.
  Future<Uint8List> deriveContactSharedSecret({
    required Uint8List x25519Shared,
    required Uint8List mlkemShared,
    required Uint8List tagSalt,
  }) async {
    _validateKeyLength(x25519Shared, 32, 'x25519Shared');
    _validateKeyLength(mlkemShared, 32, 'mlkemShared');
    _validateKeyLength(tagSalt, 32, 'tagSalt');
    final hkdf = DartHkdf(hmac: DartHmac.sha256(), outputLength: 32);
    final key = await hkdf.deriveKey(
      secretKey: SecretKey(
        Uint8List.fromList([...x25519Shared, ...mlkemShared]),
      ),
      nonce: tagSalt,
      info: utf8.encode('sealed-contact-secret-v1'),
    );
    return Uint8List.fromList(await key.extractBytes());
  }
}
