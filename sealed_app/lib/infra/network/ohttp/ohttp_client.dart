...
import 'package:sealed_app/infra/network/ohttp/ohttp_config.dart';
import 'package:sealed_app/infra/network/ohttp/ohttp_encapsulator.dart';
...
class OhttpClient {
  ...
  Future<OhttpResponse> _request(
    ...
    if (kDebugMode) {
      ...
      Log.d('[OHTTP] ① fetching gateway config from $gatewayConfigUrl');
    }
    // 1. Get gateway config
    final config = await getConfig();
    if (kDebugMode) Log.d('[OHTTP] ① config OK');
    final encapsulator = OhttpEncapsulator(config);

    // 2. Encapsulate the request
    if (kDebugMode) Log.d('[OHTTP] ② encapsulating request');
    ...
    // 3. Send to relay
    if (kDebugMode) Log.d('[OHTTP] ③ POST relay $relayUrl');
    final relayResponse = await _httpClient
        ...
        .timeout(
          ...
          onTimeout: () {
            if (kDebugMode) Log.d('[OHTTP] ✖ relay POST timed out after 20s');
    ...
    if (kDebugMode) Log.d('[OHTTP] ④ decapsulating response');
